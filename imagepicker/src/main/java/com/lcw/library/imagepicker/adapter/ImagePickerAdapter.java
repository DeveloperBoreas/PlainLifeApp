...
public class ImagePickerAdapter extends RecyclerView.Adapter<ImagePickerAdapter.BaseHolder> {
    ...
    private void bindMedia(MediaHolder mediaHolder, MediaFile mediaFile) {
        ...
        String tempImagePath = mediaFile.getPath();
        //选择状态（仅是UI表现，真正数据交给SelectionManager管理）
        if (SelectionManager.getInstance().isImageSelect(tempImagePath)) {
        ...
        try {
            imagePath = ConfigManager.getInstance().getImageLoader().loadImage(mediaHolder.mImageView, tempImagePath);
        } catch (Exception e) {
            e.printStackTrace();
        }
