...
//        File mediaStorageDir = new File(Environment.getExternalStorageDirectory().getAbsolutePath(), FILE_PATH);
        ...
        File mediaStorageDir = new File(Environment.getExternalStoragePublicDirectory(
                Environment.DIRECTORY_DCIM), filePath);
//        File mediaStorageDir = new File(Environment.getExternalStorageDirectory().getPath());
        ...
        if (type == MEDIA_TYPE_IMAGE) {
            ...
            mediaFile = new File(mediaStorageDir.getPath() + File.separator +
                    "VID_" + timeStamp + ".mp4");
        }else if(type == MEDIA_TYPE_PRIVATE_IMAGE){
            mediaFile = new File(mediaStorageDir.getPath() + File.separator +
                    "IMG_" + timeStamp + MEDIA_TYPE_PRIVATE_IMAGE_SUFFIX);
        }else if(type == MEDIA_TYPE_PRIVATE_VIDEO){
            mediaFile = new File(mediaStorageDir.getPath() + File.separator +
                    "VID_" + timeStamp + MEDIA_TYPE_PRIVATE_VIDEO_SUFFIX);
        } else {
            return null;
        }
//        try {
