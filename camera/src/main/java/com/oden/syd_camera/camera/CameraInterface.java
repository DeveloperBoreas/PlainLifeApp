...
public class CameraInterface {
    ...
    //私有图片文件
    public static final int MEDIA_TYPE_PRIVATE_IMAGE = 1234;
    public static final String MEDIA_TYPE_PRIVATE_IMAGE_SUFFIX = ".privateImage";
    //私有视频文件
    public static final int MEDIA_TYPE_PRIVATE_VIDEO = 4321;
    public static final String MEDIA_TYPE_PRIVATE_VIDEO_SUFFIX = ".privateVideo";
...
//                MainActivity.this.sendBroadcast(localIntent);
    ...
    private boolean prepareVideoRecorder(Surface surface) {
        ...
        mMediaRecorder.setOutputFile(FileUtils.getOutputMediaFile(MEDIA_TYPE_VIDEO, PRIVATE_FILE_PATH).toString());
    ...
    public void takePrivagePicture(){
        if (mCamera != null)
            mCamera.takePicture(null, null, mPrivatePicture);
    }

    private Camera.PictureCallback mPrivatePicture = new Camera.PictureCallback() {
        @Override
        public void onPictureTaken(byte[] data, Camera camera) {
            File pictureFile = FileUtils.getOutputMediaFile(MEDIA_TYPE_PRIVATE_IMAGE, PRIVATE_FILE_PATH);
            if (pictureFile == null) {
...
//                Log.d(TAG, "拍照，存储中: " + Arrays.toString(data));
    ...
    private Camera.PictureCallback mPicture = new Camera.PictureCallback() {
        @Override
        public void onPictureTaken(byte[] data, Camera camera) {
            File pictureFile = FileUtils.getOutputMediaFile(MEDIA_TYPE_IMAGE, PRIVATE_FILE_PATH);
            if (pictureFile == null) {
                Log.e(TAG, "Error creating media file, check storage permissions");
                onTakePictureFail(data);
                return;
