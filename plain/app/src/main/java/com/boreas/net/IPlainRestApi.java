...
public class IPlainRestApi implements PlainRestApi {
    ...
    public Observable<HashMap<String,ArrayList<PicEntity.Pic>>> getLocalPics() {
        return Observable.create(subscriber -> {
            if (NetWorkUtil.isNetWorkEnable(context)) {
                ArrayList<PicEntity.Pic> allArrayList = new ArrayList<>();
                HashMap<String, ArrayList<PicEntity.Pic>> groupMap = new HashMap<>();
                ...
                groupMap.put(context.getString(R.string.all_image),allArrayList);
                if (imageCursor != null && imageCursor.getCount() > 0) {
                    while (imageCursor.moveToNext()) {
                        ...
                        //获取图片的详细信息
                        String desc = imageCursor.getString(imageCursor.getColumnIndex(MediaStore.Images.Media.DESCRIPTION));
                        String parentPath = imageCursor.getString(imageCursor.getColumnIndex(MediaStore.Images.Media.DATA));
                        PicEntity.Pic picItem = new PicEntity.Pic(name, new String(data, 0, data.length - 1), parentPath);
                        groupMap.get(context.getString(R.string.all_image)).add(picItem);

                        String folderName = new File(picItem.picParentPath).getParentFile().getName();
                        if (!groupMap.containsKey(folderName)) {
                            ArrayList<PicEntity.Pic> arrayList = new ArrayList<>();
                            arrayList.add(picItem);
                            groupMap.put(folderName, arrayList);
                        } else {
                            groupMap.get(folderName).add(picItem);
                        }
                    }
                    if(groupMap != null && groupMap.size() > 0){
                        subscriber.onNext(groupMap);
                        subscriber.onCompleted();
                    }else{
    ...
    private void getPicList(Subscriber subscriber) {
        try {
            ...
            while (cursor.moveToNext()) {
                ...
                String parentPath = cursor.getString(cursor.getColumnIndex(MediaStore.Images.Media.DATA));

                list.add(new PicEntity.Pic(name, new String(data, 0, data.length - 1),parentPath));
            }
