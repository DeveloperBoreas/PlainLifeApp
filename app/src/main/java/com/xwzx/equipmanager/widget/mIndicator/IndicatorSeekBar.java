package com.xwzx.equipmanager.widget.mIndicator;
...
public class IndicatorSeekBar extends View {
    ...
    private boolean mShowTimeTickText;//the palace where the tick text show .
    ...
    private String[] mTickTimeTextsArr;//save the tick texts which at tickMark position.
    private float[] mTickTextsWidth;//save the tick texts width bounds.
    private float[] mTickTimeTextsWidth;//save the tick texts width bounds.
    private float[] mTextCenterX;//the text's drawing X anchor
    private float[] mTimeTextCenterX;//the text's drawing X anchor
    private float mTickTextY;//the text's drawing Y anchor
    private float mTimeTickTextY;//the text's drawing Y anchor
    ...
    private CharSequence[] mTickTimeTextsCustomArray;
    //indicator
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mShowTimeTickText = ta.getBoolean(R.styleable.IndicatorSeekBar_isb_show_time_tick_texts, builder.showTimeTickText);
        ...
        mTickTimeTextsCustomArray = ta.getTextArray(R.styleable.IndicatorSeekBar_isb_tick_time_texts_array);
    ...
    private void collectTicksInfo() {
        ...
        if (mTicksCount != 0) {
            ...
            if(mShowTimeTickText){
                mTimeTextCenterX = new float[mTicksCount];
                mTickTimeTextsWidth = new float[mTicksCount];
            }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(resolveSize(SizeUtils.dp2px(mContext, 170), widthMeasureSpec), height + mTickTextsHeight+mTickTextsHeight);
        initSeekBarInfo();
    ...
    private void refreshSeekBarLocation() {
        ...
        if (needDrawText()) {
            ...
            mTimeTickTextY = mTickTextY+ Math.round(mRect.height() - mTextPaint.descent()) + SizeUtils.dp2px(mContext, 3);
    ...
    private void initTimeTextsArray() {
        if (mTicksCount == 0) {
            return;
        }
        if (mShowTimeTickText) {
            mTickTimeTextsArr = new String[mTicksCount];
        }
        for (int i = 0; i < mTickMarksX.length; i++) {
            if (mShowTimeTickText) {
                mTickTimeTextsArr[i] = getTickTimeTextByPosition(i);
                if(mTextPaint!=null&&mRect!=null){
                    mTextPaint.getTextBounds(mTickTimeTextsArr[i], 0, mTickTimeTextsArr[i].length(), mRect);
                    mTickTimeTextsWidth[i] = mRect.width();
                }
//                mTimeTextCenterX[i] = mPaddingLeft + mSeekBlockLength * i;
                mTimeTextCenterX[i] = mPaddingLeft + mSeekBlockLength * i;
            }
//            mTickMarksX[i] = mPaddingLeft + mSeekBlockLength * i;
        }
    ...
    private String getTickTimeTextByPosition(int index) {
        if (mTickTimeTextsCustomArray == null) {
            return getProgressString(mProgressArr[index]);
        }
        if (index < mTickTimeTextsCustomArray.length) {
            return String.valueOf(mTickTimeTextsCustomArray[index]);
        }
        return "";
    }
    ...
    private void drawTimeTickTexts(Canvas canvas) {
        if (mTickTimeTextsArr == null) {
            return;
        }
        float thumbPosFloat = getThumbPosOnTickFloat();
        for (int i = 0; i < mTickTimeTextsArr.length; i++) {
            if (mShowBothTickTextsOnly) {
                if (i != 0 && i != mTickTimeTextsArr.length - 1) {
                    continue;
                }
            }
            if (i == getThumbPosOnTick() && i == thumbPosFloat) {
            ...
            int index = i;
            if (mR2L) {
                index = mTickTimeTextsArr.length - i - 1;
            }
            if (i == 0) {
                canvas.drawText(mTickTimeTextsArr[index], mTimeTextCenterX[i] + mTickTimeTextsWidth[index] / 2.0f, mTimeTickTextY, mTextPaint);
            } else if (i == mTickTimeTextsArr.length - 1) {
                canvas.drawText(mTickTimeTextsArr[index], mTimeTextCenterX[i] - mTickTimeTextsWidth[index] / 2.0f, mTimeTickTextY, mTextPaint);
            } else {
                canvas.drawText(mTickTimeTextsArr[index], mTimeTextCenterX[i]+ mTickTimeTextsWidth[index] / 2.0f, mTimeTickTextY, mTextPaint);
            }
    ...
    private void apply(Builder builder) {
        ...
        this.mShowTimeTickText = builder.showTimeTickText;
        this.mTickTextsSize = builder.tickTextsSize;
        this.mTickTextsCustomArray = builder.tickTextsCustomArray;
        this.mTickTimeTextsCustomArray = builder.tickTimeTextsCustomArray;
    ...
    public void customTickTimeTexts(@NonNull String[] tickTextsArr) {
        this.mTickTimeTextsCustomArray = tickTextsArr;
        if (mTickTimeTextsArr != null) {
            for (int i = 0; i < mTickTimeTextsArr.length; i++) {
                String tickText;
                if (i < tickTextsArr.length) {
                ...
                mTickTimeTextsArr[index] = tickText;
                if (mTextPaint != null && mRect != null) {
                    mTextPaint.getTextBounds(tickText, 0, tickText.length(), mRect);
                    mTickTimeTextsWidth[index] = mRect.width();
                }
            }
            invalidate();
        }
